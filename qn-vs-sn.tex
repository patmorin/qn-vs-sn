...
\begin{compactenum}[(i)]
  \item ($d$-monotone): There is no $(d+1)$-element edge set $\{v_0w_0,\ldots,v_{d}w_d\}\subseteq E(G)$ such that $v_0<v_1<\cdots < v_d$ and $w_0 > w_1>\cdots>w_d$.
...
We consider the following $(2r+1)n$-vertex graph $X_{n,r}$:
\begin{itemize}
...
\begin{lem}
  Let $\epsilon$ be defined as in \cref{bourgain}.
  For any integers $k,p,n,r$ with $\epsilon k/2 \ge p$, $kp\le n$, $r\ge \log(n/k)/\log(1+\epsilon/2)$, the following statement about the graph $X:=X_{n,r}$ is true:

  For any $k$-element subsets $S\subseteq V_0$ and $T\subseteq V_{2r}$, there exists $\epsilon k/2$ vertex-disjoint paths $P_1,\ldots,P_{\alpha k}$ each of length $2r$ each having an endpoint in $S$ and an endpoint in $T$.
\end{lem}

\begin{proof}
The proof is just like the proof below.
\end{proof}


\begin{lem}[Now defunct]\label{x_graph}
  Let $\epsilon$ be defined as in \cref{bourgain}.
  For any integers $k,p,n,r$ with $\epsilon k/2 \ge p$, $kp\le n$, $r\ge \log(n/k)/\log(1+\epsilon/2)$, the following statement about the graph $X:=X_{n,r}$ is true:

  For any pairwise disjoint $k$-element subsets $S_1,\ldots,S_p\subseteq V_0$ and $T_1,\ldots,T_p\subseteq V_{2r}$, the graph $X$ contains vertex-disjoint paths $P_1,\ldots,P_p$ each of length $2r$ and, for each $\ell\in\{1,\ldots,p\}$, $P_\ell$ has endpoints in $S_\ell$ and $T_\ell$.
\end{lem}

\begin{proof}
  We will construct the paths $P_1,\ldots,P_p$ one at a time.  To construct the path $P_\ell$ after constructing $P_1,\ldots,P_{\ell-1}$, we let $\overline{V}:=\bigcup_{a=1}^{\ell-1} V(P_a)$ be the set of vertices that must be avoided by $P_\ell$.

  Let $S_0':=S_\ell$.  For each $i\in\{1,\ldots,r\}$, define $S_i':=N_X(V_{i-1}')\cap V_i\setminus \overline{V}$.  Observe that, for each $v\in S_i'$, there exists a length-$i$ path $P_v$ from $v$ to some vertex in $S_0'=S_\ell$.  We claim that, for each $i\in\{0,\ldots,r\}$, $|S_i'|\ge \min\{n/2+1,(1+\epsilon/2)k\}$ and prove this claim by induction on $i$.  In the base case $i=0$, and the statement is easily verified since $|S_0'|=|S_\ell|=k=(1+\epsilon/2)^0k$.

  Now suppose $i\in\{1,\ldots,r\}$.  If $|V_{i-1}'|\ge n/2+1$ then $|S_i'|\ge |V_{i-1}|\ge n/2+1$ and we are done.  Otherwise, $|V_{i-1}'|\le n/2$ and, since the graph $X[V_{i-1}\cup V_i]$ is isomorphic to an $\epsilon$-expander,  $|N_X(S_{i-1}')\cap V_i| \ge (1+\epsilon)|S_{i-1}'|\ge (1+\epsilon)(1+\epsilon/2)^{i-1}k$.  Furthermore,
  \[
    |S_i'| = |N_X(S_{i-1'})\cap V_i\setminus \overline{V}|
      \ge |N_X(S_{i-1}')\cap V_i|-p
      \ge (1+\epsilon)(1+\epsilon/2)^{i-1}k - p
      \ge (1+\epsilon/2)^ik \enspace ,
  \]
  This implies that
  \[
    |S_r'|\ge \min\{n/2+1,(1+\epsilon/2)^rk\} = \min\{n/2+1,n\} = n/2+1
  \]
  since $r\ge \log(n/k)/\log(1+\epsilon/2)$.

  A symmetric argument shows that there exists a set $T_r'\subseteq V_r$, $|T_r'|\ge n/2+1$ such that, for each $v\in T_r'$ there exists a length-$r$ path $Q_v$ from $v$ to some vertex in $T_\ell$.  Since $S_r',T_r'\subseteq V_r$ and $|S_r'|,|T_r'|\ge n/2+1$, $S_r'\cap T_r'$ is non empty.  Let $v$ be any vertex in $S_r'\cap T_r'$ and observe that the union of the two paths $P_v$ and $Q_v$ gives a length-$2r$ path $P_\ell$ from some vertex in $S_\ell$ to some vertex in $T_\ell$ that is vertex-disjoint from every path in $P_1,\ldots,P_{\ell-1}$.
\end{proof}
